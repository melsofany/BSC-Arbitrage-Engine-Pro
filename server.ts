...
let ethcallProvider: MulticallProvider | null = null;
let wsProvider: WebSocket | null = null;
...
async function initMulticall() {
  try {
    ...
    // @ts-ignore
    ethcallProvider = new MulticallProvider(chainId, provider);
    console.log(`Multicall Provider initialized for chain ${chainId}`);
  } catch (e: any) {
...
// Mempool Listener for MEV
function setupMempoolListener() {
  if (wsProvider) wsProvider.close();
  
  const wsUrl = WS_NODES[0];
  console.log(`Connecting to Mempool WebSocket: ${wsUrl}`);
  
  wsProvider = new WebSocket(wsUrl);
  ...
  wsProvider.on("message", async (data) => {
    try {
      const message = JSON.parse(data.toString());
      if (message.params && message.params.result) {
        const txHash = message.params.result;
        // Logic to fetch tx details and simulate impact
  ...
  wsProvider.on("close", () => {
    ...
    setTimeout(setupMempoolListener, 5000);
  });
}

setupMempoolListener();
    ...
    if (ethcallProvider) {
      ...
      for (const [pairName, [tA, tB]] of Object.entries(tokenPairs)) {
        for (const [dexName, routerAddr] of Object.entries(routers)) {
          const multicallContract = new MulticallContract(routerAddr, ROUTER_ABI as any);
          calls.push(multicallContract.getAmountsOut(amountIn, [tA, tB]));
          callMap.push({ pairName, dexName });
        }
      }

      // @ts-ignore
      const resultsArray = await ethcallProvider.all(calls).catch(e => {
      ...
      if (resultsArray) {
        const results: any = {};
        resultsArray.forEach((amounts: any, index: number) => {
          const { pairName, dexName } = callMap[index];
          if (!results[pairName]) results[pairName] = {};
...
app.post("/api/verify-contract", express.json(), async (req, res) => {
...
app.post("/api/wallet-balance", express.json(), async (req, res) => {
...
app.post("/api/settings/advanced", express.json(), async (req, res) => {
  const { privateRpc, useMevShare, privateKey } = req.body;
...
app.post("/api/execute", express.json(), async (req, res) => {
  ...
  const minProfitPercent = parseFloat(rawMinProfit || "0.5");
  ...
  try {
    ...
    const [tokenA, tokenB] = tokenPairs[pair || "WBNB/BUSD"] || [WBNB, BUSD];
    ...
    // Determine the correct direction based on prices using a small amount to avoid slippage bias
    let borrowToken = tokenA;
    let outToken = tokenB;
    let borrowDecimals = decimalsA;
    let outDecimals = decimalsB;
    
    // Use 1 unit of tokenA for price comparison
    const checkAmount = ethers.parseUnits("1", decimalsA);

    try {
      const buyContract = new ethers.Contract(buyRouterAddr, ["function getAmountsOut(uint256, address[]) view returns (uint256[])"], provider);
      const sellContract = new ethers.Contract(sellRouterAddr, ["function getAmountsOut(uint256, address[]) view returns (uint256[])"], provider);
      
      const [buyAmountsA, sellAmountsA] = await Promise.all([
        buyContract.getAmountsOut(checkAmount, [tokenA, tokenB]).catch(() => null),
        sellContract.getAmountsOut(checkAmount, [tokenA, tokenB]).catch(() => null)
      ]);
      
      if (buyAmountsA && sellAmountsA) {
        const buyOutA = buyAmountsA[buyAmountsA.length - 1];
        const sellOutA = sellAmountsA[sellAmountsA.length - 1];
        
        console.log(`Price Check (for 1 unit):
        - ${buyDex}: ${ethers.formatUnits(buyOutA, decimalsB)}
        - ${sellDex}: ${ethers.formatUnits(sellOutA, decimalsB)}`);

        if (buyOutA > sellOutA) {
          // 1 tokenA gets more tokenB on BuyDex -> tokenB is cheaper on BuyDex
          borrowToken = tokenA;
          outToken = tokenB;
          borrowDecimals = decimalsA;
          outDecimals = decimalsB;
        } else {
          // 1 tokenA gets more tokenB on SellDex -> tokenA is cheaper on BuyDex
          borrowToken = tokenB;
          outToken = tokenA;
          borrowDecimals = decimalsB;
          outDecimals = decimalsA;
        }
      }
    } catch (e) {
      console.error("Direction check failed:", e);
    }
    ...
    try {
      ...
      let profit = isProfitable ? finalAmount - amountToRepay : 0n;
      ...
      - Max Theoretical Spread: ${maxPossibleBps} bps (after 0.9% fees)
      ...
      if (profitBps < BigInt(minProfitBps)) {
        ...
        if (maxPossibleBps < BigInt(minProfitBps)) {
          console.log(`❌ Even at spot price (no slippage), profit (${maxPossibleBps} bps) is below requirement (${minProfitBps} bps).`);
        }
        ...
        try {
          ...
          if (bestAmount > 0n) {
            ...
            profit = finalAmount - amountToRepay;
      ...
      console.log(`Theoretical Profit: ${ethers.formatUnits(profit, borrowDecimals)} ${borrowToken} (${profitBps} bps)`);
    } catch (e: any) {
